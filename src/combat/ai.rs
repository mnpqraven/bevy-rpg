...
use crate::{ecs::component::*, combat::setup::setup_target_bucket};

use super::{
    process::TurnOrderList,
    ChooseAISkillEvent,
};
pub struct AiPlugin;
...
fn choose_skill(
    ...
    for _ in ev_choose_ai_skill.iter() {
        if !&turn_order.is_empty() {
            ...
            let pool: Vec<(Entity, &SkillGroupList, &Target)> =
                skill_q.iter().filter(|item| item.1.0.contains(&filter)).collect();
            ...
            let filtered_units = setup_target_bucket(
                unit_q.to_readonly(),
                target_type,
                *turn_order.get_current().expect("turnorder should be generated by now"),
                unit_q.get(*turn_order.get_current().unwrap()).unwrap().3.is_some()
            );
